...
class ExemplarsSelector:
    ...
    def __call__(self, model: LLL_Net, t: int, trn_loader: DataLoader, transform):
        clock0 = time.time()
        ...
        with override_dataset_transform(trn_loader.dataset, transform) as ds_for_selection:
            ...
            selected_indices = self._select_indices(model, t, sel_loader, exemplars_per_class, transform)
    ...
    def _select_indices(self, model: LLL_Net, t: int, sel_loader: DataLoader, exemplars_per_class: int, transform) -> Iterable:
        pass
    ...
    def _select_indices(self, model: LLL_Net, t: int, sel_loader: DataLoader, exemplars_per_class: int, transform) -> Iterable:
        ...
        with torch.no_grad():
            ...
            for images, targets in sel_loader:
                model.apply(lambda m: setattr(m, 'shift', 1*t))
