...
class LLL_Net(nn.Module):
    ...
    def __init__(self, model, remove_existing_head=False):
        ...
        assert not remove_existing_head or type(getattr(model, head_var)) in [nn.Sequential, nn.Linear, SlimmableLinear], \
        ...
        if remove_existing_head:
            ...
            elif type(last_layer) == SlimmableLinear:
                self.out_size = last_layer.in_features
                setattr(self.model, head_var, nn.Sequential())
        else:
    ...
    def add_head(self, num_outputs):
        ...
        """
        Ch_in = [int(self.out_size * width_mult) for width_mult in FLAGS.width_mult_list]
        Ch_out = [num_outputs for width_mult in FLAGS.width_mult_list]

        self.heads.append(SlimmableLinear(Ch_in, Ch_out))
